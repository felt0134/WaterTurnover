...
#-----------------------------------------------
# Resample according to region of interest--------------------------
...
get_regional_turnover<-function(region,x){
  ...
  for(i in et_9km_monthly_2002_2017_length[x]){
    ...
    if(region =='Mexico'){
      ...
      wd<-subset(density_region_mean,region=='Mexico')
      wd<-round(wd$wd,2)
      
      #upload region shapefile and raster
      shapefile <- wrld_simpl[wrld_simpl$NAME %in% c('Mexico'),]
      region.raster<-raster('./../../Data/Derived_data/Biomass/Region/Mexico_AGB.tif')
      
      #Mexico Canopy Transpiration
      ET<-region_resample(shapefile,region.raster,et.data.9km)
      #head(Mexico_ET)
      ...
      #re-scale
      region_biomass$Mexico_AGB<- 0.1*region_biomass$Mexico_AGB
      region_biomass$AGB<-region_biomass$Mexico_AGB
      
    }else if(region=='China'){
      
      wd<-subset(density_region_mean,region=='China')
      wd<-round(wd$wd,2)
      
      #upload region shapefile and raster
      shapefile <- wrld_simpl[wrld_simpl$NAME %in% c('China'),]
      region.raster<-raster('./../../Data/Derived_data/Biomass/Region/China_AGB.tif')
      ...
      #re-scale
      region_biomass$China_AGB<- 0.1*region_biomass$China_AGB
      region_biomass$AGB<-region_biomass$China_AGB}else if(region=='Madagascar'){
        
        wd<-subset(density_region_mean,region=='Madagascar')
        wd<-round(wd$wd,2)
        
        #upload region shapefile and raster
        shapefile <- wrld_simpl[wrld_simpl$NAME %in% c('Madagascar'),]
        region.raster<-raster('./../../Data/Derived_data/Biomass/Region/Madagascar_AGB.tif')
        ...
        #re-scale
        region_biomass$Madagascar_AGB<- 0.1*region_biomass$Madagascar_AGB
        region_biomass$AGB<-region_biomass$Madagascar_AGB}else if(region=='India'){
          
          wd<-subset(density_region_mean,region=='India')
          wd<-round(wd$wd,2)
          
          #upload region shapefile and raster
          shapefile <- wrld_simpl[wrld_simpl$NAME %in% c('India'),]
          region.raster<-raster('./../../Data/Derived_data/Biomass/Region/India_AGB.tif')
          ...
          #re-scale
          region_biomass$India_AGB<- 0.1*region_biomass$India_AGB
          region_biomass$AGB<-region_biomass$India_AGB}else if(region=='Australia'){
            
            wd<-subset(density_region_mean,region=='Australia')
            wd<-round(wd$wd,2)
            
            #upload region shapefile and raster
            shapefile <- wrld_simpl[wrld_simpl$NAME %in% c('Australia'),]
            region.raster<-raster('./../../Data/Derived_data/Biomass/Region/Australia_AGB.tif')
            ...
            #re-scale
            region_biomass$Australia_AGB<- 0.1*region_biomass$Australia_AGB
            region_biomass$AGB<-region_biomass$Australia_AGB}else if(region=='NorthAmerica'){
              
              wd<-subset(density_region_mean,region=='NorthAmerica')
              wd<-round(wd$wd,2)
              ...
              #re-scale
              region_biomass$Australia_AGB<- 0.1*region_biomass$Australia_AGB
              region_biomass$AGB<-region_biomass$Australia_AGB}else if(region=='Europe'){
                
                wd<-subset(density_region_mean,region=='Europe')
                wd<-round(wd$wd,2)
                
                #upload region shapefile and raster
                shapefile <- wrld_simpl[wrld_simpl$NAME %in% c('Russia','Germany','United Kingdon',
                                                               'France','Italy','Spain','Ukraine',
                                                               'Poland','Romania','Netherlands',
                                                               'Belgium','Czech Republic','Greece','Iceland',
                                                               'Portugal','Sweden','Hungary','Belarus',
                                                               'Austria','Serbia','Switzerland','Bulgaria',
                                                               'Denmark','Finland','Slovakia','Norway',
                                                               'Ireland','Croatia','Moldova','Bosnia and Herzegovina',
                                                               'Albania','Lithuania','Slovenia','Latvia',
                                                               'Estonia','Montenegro','Luxembourg','Malta','North Macedonia',
                                                               'Andorra','Monaco','Liechtenstein',
                                                               'San Marino','Holy See'),]
                
                region.raster<-raster('./../../Data/Derived_data/Biomass/Region/Europe_AGB.tif')
                ...
                #re-scale
                region_biomass$Europe_AGB<- 0.1*region_biomass$Europe_AGB
                region_biomass$AGB<-region_biomass$Europe_AGB}else if(region==xxx){}
    ...
    region_biomass$wood_water_grams_m2<- ((1-wd)/wd)*region_biomass$AGB
    region_biomass$wood_water_grams_m2<-round(region_biomass$wood_water_grams_m2,2)
    ...
    region_biomass$wood_water_mm<-region_biomass$wood_water_grams_m2*0.001
    region_biomass$wood_water_mm <- round(region_biomass$wood_water_mm,2)
    ...
    #merge with ET data
    region_biomass_woodwater_transpiration<-merge(region_biomass,ET,by=c('x','y'))
    
    rm(ET,region_biomass)
    
    #get rid of excess colums
    region_biomass_woodwater_transpiration<-region_biomass_woodwater_transpiration[c(1,2,6,7)]
    
    #rename columns
    colnames(region_biomass_woodwater_transpiration) <-c('x','y','water_storage_mm_m2','canopy_transpiration_mm_m2')
    
    #convert latent energy to mm
    region_biomass_woodwater_transpiration<-aggregate(canopy_transpiration_mm_m2~x+y+water_storage_mm_m2,
                                                      le_to_cumulative_monthly_mm,data=region_biomass_woodwater_transpiration)
    
    #filter our zero values
    region_biomass_woodwater_transpiration <- region_biomass_woodwater_transpiration %>%
      dplyr::filter(water_storage_mm_m2 > 0)
    
    #store in list
    turnover.list[[i]] <- region_biomass_woodwater_transpiration
