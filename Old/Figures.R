...
#-------------------------------------------------------------------------------
#climate effects coefficient distributions-----

aridity_effect <- read.csv('./../../../Data/Derived_data/Model_outputs/annual_aridity_coef.csv')
summary(aridity_effect)
head(aridity_effect)

aggregate(climate_mean~veg,mean,data=aridity_effect)

#look at 2.5 and 97.5, take a max likelihood approach
ggplot(aridity_effect,aes(x=climate_mean,fill=veg)) +
  scale_y_continuous(expand = c(0,0),limits = c(0,1.02)) +
