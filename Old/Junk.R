...
#------------------------------------------------
# Stratify data by latitude, run, spatial error models, store coefficients for transit-climate spatial relationships-----

run_spatial_models <- function(data,x,log=T){
  
  #list to store outputs
  # coefficient_annual_list <- list()
  # sample_size_annual_list <- list()
  ...
  test <- data.frame(data[x])
  colnames(test) <- c('x', 'y', 'cover', 'transit', 'climate_mean')
  test<- test %>%
    dplyr::filter(transit > 0)
  #head(test)
  
  test.strat_annual<-stratified(test, c("y"), 0.002)
  test.strat_annual$transit <- round(test.strat_annual$transit,2)
  test.strat_annual$climate_mean <- round(test.strat_annual$climate_mean,2)
  #head(test.strat_annual)
  
  sample_sizes_annual<-aggregate(transit~cover,length,data=test.strat_annual)
  
  #visualize various componenets:
  
  #land cover spefici bivariate relationships
  
  # ggplot(test.strat_annual,aes(climate_mean,transformed(transit))) +
  ...
  # xyzSP <- SpatialPointsDataFrame(coords = data.frame(test.strat_annual)[,1:2],
  #                                 data = data.frame(test.strat_annual),
  #                                 proj4string = CRS("+proj=longlat +datum=WGS84 +no_defs"))
  # 
  # plot(xyzSP,cex=0.1,main='Dimension Reduction')
  # box()
  
  #model 1: just aridity
  # model_1_annual <- lm(transit~climate_mean,data=test.strat_annual)
  # plot(model_1_annual)
  # summary(model_1_annual)
  # #transform to better meet assumptions:
  # #square root transform better meets assumptions than log transform 
  # 
  # model_1_annual_transformed <- lm(log(transit)~climate_mean,data=test.strat_annual)
  # plot(model_1_annual_transformed)
  # # summary(model_1_annual_transformed)
  # # AIC(model_1_annual_transformed)
  # #low r-squared
  # 
  # #model 2: influence of aridity by land cover type
  # model_2_annual <- lm(transit~climate_mean + cover + climate_mean:cover,data=test.strat_annual)
  # plot(model_2_annual)
  ...
  if(log==T){
    ...
    model_2_annual_transformed <- lm(log(transit)~climate_mean + cover + climate_mean:cover,data=test.strat_annual)
    #plot(model_2_annual_transformed)
    #summary(model_2_annual_transformed)
    #AIC(model_2_annual_transformed)
    
    #now look at autocorrelation and create spatial weight matrix
    test.strat_annual$Yresid <- resid(model_2_annual_transformed)
    
    #cor(test.strat_annual$layer,test.strat_annual$mean_aridity)
    
    #variogramL visualize autocorrelation
    # Vout_annual<-variogram(Yresid~1,loc=~x+y, width=.05,data=test.strat_annual) ###calculate a variogram using the data. this will take a few annualutes if the full dataset
    # plot(Vout_annual$dist,Vout_annual$gamma,xlab='Distance (Deg)', ylab='Semivariance',main='',cex.lab=1.5, cex.main=2, pch=19, cex=.5)
    
    #prep spatial weights
    poly.resids_annual<-rasterToPolygons(rasterFromXYZ(test.strat_annual[,c(1,2,6)]))
    nb_annual <- poly2nb(poly.resids_annual, queen=TRUE)
    lw_annual <- nb2listw(nb_annual, style="W", zero.policy=TRUE)
    #lw_annual$weights
    
    # do tests
    # moran.test(poly.resids_annual$Yresid, lw_annual, alternative="greater",zero.policy=TRUE)
    # # often suggests no autocorrelation
    # moran.mc(poly.resids_annual$Yresid, lw_annual,nsim=999, alternative="greater",zero.policy=TRUE)
    # often suggests autocorrelation
    
    #to be conservative, we proceed with spatial error models to account for any
    #potential autocorrelation in each randomized stratified iteration
    
    #spatial error model
    spatial_error_annual <- errorsarlm(log(transit)~climate_mean + cover + climate_mean:cover,
                                       data=test.strat_annual,listw = lw_annual,
                                       zero.policy = T)
    #summary(spatial_error_annual)
    
    #spatial lag model
    # spatial_lag_annual<-lagsarlm(transformed(transit)~climate + cover + climate:cover,
    #                           data=test.strat_annual,lw_annual,zero.policy = T)
    # summary(spatial_lag_annual)
    ...
    model_2_annual_transformed <- lm(sqrt(transit)~climate_mean + cover + climate_mean:cover,
                                     data=test.strat_annual)
    ...
    #spatial error model
    spatial_error_annual <- errorsarlm(sqrt(transit)~climate_mean + cover + climate_mean:cover,
                                       data=test.strat_annual,listw = lw_annual,
                                       zero.policy = T)
  ...
  coef.df<-data.frame(coef(spatial_error_annual))
  #head(coef.df)
  
  #transpose (t) to from long to wide dataframe
  test.wide<-data.frame(t(coef.df))
  
  #fix columns values, need to add/subtract from grassland
  test.wide$grassland.climate.int <- test.wide$climate_mean
  test.wide$forest.climate.int <- test.wide$climate_mean + test.wide$climate_mean.coverforest
  test.wide$shrubland.climate.int <- test.wide$climate_mean + test.wide$climate_mean.covershrubland
  test.wide$tundra.climate.int <- test.wide$climate_mean + test.wide$climate_mean.covertundra
  test.wide$cropland.climate.int <- test.wide$climate_mean + test.wide$climate_mean.covercropland
  ...
  # coefficient_annual_list[[i]] <- test.wide
  # sample_size_annual_list[[i]] <- sample_sizes_annual
  
  
  return(list(test.wide,sample_sizes_annual))
...
plot(merge(grasslands_unfiltered, shrublands_unfiltered,tundras_unfiltered,forests_unfiltered,
           croplands_unfiltered))
plot(forests_unfiltered)
#get  summary stats
